...
exports.log = function(error, response) {
	if (undefined === response) {
		if (null === error || undefined === error) {
			c.LOG.info('<AddRoleOrMoveUser API INFO> ' + getTimestamp() + '  Successful API Call');
		} else {
			c.LOG.info('<AddRoleOrMoveUser API RESPONSE> ' + getTimestamp() + '  ERROR: ' + error);			
		}
	} else {
		c.LOG.info('<AddRoleOrMoveUser API RESPONSE> ' + getTimestamp() + '  ' + response);
	}
}
